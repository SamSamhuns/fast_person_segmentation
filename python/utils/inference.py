...
class ImageioVideoWriter(object):
    def __init__(
        self, output_dir: str, video_name: str, model_fname: str, fps: int = 25
    ):
...
def get_cmd_argparser(
    default_model="models/transpose_seg/deconv_bnoptimized_munet_e260.hdf5",
):
    """
...
def remove_argparse_option(parser, arg):
    ...
    for action in parser._actions:
        if (
            vars(action)["option_strings"] and vars(action)["option_strings"][0] == arg
...
def load_bgd(
    ...
    bg_h: int,
    dtype=np.float32,
    post_process: Callable = _default_bg_load_transform,
) -> np.ndarray:
    """
