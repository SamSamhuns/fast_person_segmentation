...
for item in dirs_img:
    if os.path.isfile(img_path + item):
        ...
        if img.shape[0] > max_height or img.shape[1] > max_width:
            ...
            msk = cv2.resize(msk, (max_height, max_width),
        ...
        for det in dets:
            ...
            if img_crop.shape[0] > min_crop_height or img_crop.shape[1] > min_crop_width:
                if len(detector(img_crop)) == 1:  # Check if there are multiple faces
                    cv2.imwrite(crop_img_path + "port_" + str(fcount) + item,
                                cv2.resize(img_crop, (target_height, target_width)))
                    cv2.imwrite(crop_msk_path + "port_" + str(fcount) + png_msk, cv2.resize(
                        msk_crop, (target_height, target_width), interpolation=cv2.INTER_NEAREST))
                    fcount = fcount + 1
'''
