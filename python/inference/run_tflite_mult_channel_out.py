...
def inference_model(
    ...
    while ret:
        ...
        if not use_prev_msk:
            ...
            msk = np.exp(out[:, :, out_channel_index]) / np.sum(np.exp(out), axis=2)
            if prev_mask is not None:
                uncertainty_alpha = 1.0 + (
                    msk * np.log(msk + eps) + (1.0 - msk) * np.log(1.0 - msk + eps)
                ) / np.log(2.0)
                ...
                mixed_mask_value = msk * uncertainty_alpha + prev_mask * (
                    1.0 - uncertainty_alpha
                )
                msk = (
