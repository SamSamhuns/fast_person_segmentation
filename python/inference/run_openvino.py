...
from utils.inference import (
    ...
    remove_argparse_option,
)
from utils.inference import (
    PostProcessingType,
    ImageioVideoWriter,
    get_video_stream_widget,
)


def get_openvino_core_net_exec(
    model_xml_path: str, model_bin_path: str, target_device: str = "CPU"
):
    # load IECore object
    OpenVinoIE = IECore()
...
def inference_video(
    ...
    while ret:
        ...
        if (
            config_dict["ov_bin"]
            == "models/selfie_seg/144x256/openvino/FP32/144x256/model_v2.bin"
        ):
            msk = out[1, :, :]
        else:
        ...
        if post_processing == PostProcessingType.MORPH_OPEN:
            kernel = cv2.getStructuringElement(
                shape=cv2.MORPH_RECT, ksize=(default_mopen_ksize, default_mopen_ksize)
            )
            msk = cv2.morphologyEx(
                msk, cv2.MORPH_OPEN, kernel=kernel, iterations=default_mopen_iter
            )
