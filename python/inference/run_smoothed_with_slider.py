...
def inference_video(
    ...
    cv2.createTrackbar(
        "threshold", cv2_disp_name, default_threshold, 20, nothing
    )  # default 12/20 = 0.6
    cv2.createTrackbar(
        "skip_frame", cv2_disp_name, default_skip_frame, 5, nothing
    )  # default 2
    cv2.createTrackbar(
        "mopen_ksize", cv2_disp_name, default_mopen_ksize, 20, nothing
    )  # default 2
    cv2.createTrackbar(
        "mopen_iter", cv2_disp_name, default_mopen_iter, 20, nothing
    )  # default 2
    ...
    while ret:
        ...
        if skip_frame == 0 or COUNT % skip_frame == 0:
            ...
            msk = cv2.morphologyEx(
                msk, cv2.MORPH_OPEN, (mopen_kernel, mopen_kernel), iterations=mopen_iter
            )
            COUNT = 0
        COUNT += 1
    ...
    print(
        f"mopen_ksize={mopen_kernel}",
        f"mopen_iter={mopen_iter}",
