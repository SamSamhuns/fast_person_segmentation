...
void Model::run(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs)
{
    ...
    this->error_check(std::all_of(inputs.begin(), inputs.end(), [](const Tensor *i)
                                  { return i->flag == 1; }),
                      "Error: Not all elements from the inputs are full");

    this->error_check(std::all_of(outputs.begin(), outputs.end(), [](const Tensor *o)
                                  { return o->flag != -1; }),
                      "Error: Not all outputs Tensors are valid");

    // Clean previous stored outputs
    std::for_each(outputs.begin(), outputs.end(), [](Tensor *o)
                  { o->clean(); });

    // Get input operations
    std::vector<TF_Output> io(inputs.size());
    std::transform(inputs.begin(), inputs.end(), io.begin(), [](const Tensor *i)
                   { return i->op; });

    // Get input values
    std::vector<TF_Tensor *> iv(inputs.size());
    std::transform(inputs.begin(), inputs.end(), iv.begin(), [](const Tensor *i)
                   { return i->val; });

    // Get output operations
    std::vector<TF_Output> oo(outputs.size());
    std::transform(outputs.begin(), outputs.end(), oo.begin(), [](const Tensor *o)
                   { return o->op; });
    ...
    // Mark input as empty
    std::for_each(inputs.begin(), inputs.end(), [](Tensor *i)
                  { i->clean(); });
