...
int test_mode(Settings &settings, char *img_name);
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Invalid number of args. FMT is ./example img <tflite_model_path> <img_path> or "
...
int test_mode(Settings &settings, char *img_name) {
  std::cout << "Starting image inference on " << img_name << "\n";
  ...
  std::tie(input_shape, output_shape) = get_input_output_dims(settings, interpreter);
  ...
  cv::resize(img, inp, cv::Size(input_shape.width, input_shape.height), cv::INTER_LINEAR);
  ...
  memcpy(interpreter->typed_input_tensor<float>(input_shape.index), inp.data, inp.total() * inp.elemSize());
  ...
  cv::Mat two_channel(output_shape.height, output_shape.width, CV_32FC2, output_data_ptr);
...
//   int wanted_bsize = in_dims->data[0];
